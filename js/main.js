document.addEventListener('DOMContentLoaded', () => {
  ...
  // Define boot log lines (type + text)
  const bootLines = [
  ...
  // Append logs one by one with delay
  const logDelay = 100; // ms per log
  bootLines.forEach((line, i) => {
    const el = document.createElement('div');
    el.className = `boot-log ${line.type}`;
    el.textContent = ` ${line.text}`;
    el.style.animationDelay = `${i * logDelay}ms`;
    // Use a small timeout to ensure the delay is applied before starting animation
    setTimeout(() => {
  ...
  // Memory diagnostic blocks
  const blocks = 200; // adjust for length
  if (memContainer) {
  ...
  // When progress completes, we enable interactions
  progressBar.addEventListener('animationend', () => {
    // Status and prompt are already animated via CSS delays.
  });
    ...
    if (bootLogo && menuHeader) {
      ...
      // sync glow between logo and nickname
  ...
  // Allow only mouse click or Enter after everything is loaded
  ...
  const enableContinue = () => {
    ...
    const clickHandler = () => {
      ...
      if (bgAudio) { try { bgAudio.volume = 0.05; bgAudio.play().catch(()=>{}); } catch(e){} }
      finishBoot();
    };
    const keyHandler = (e) => {
      if (e.key === 'Enter') {
  ...
  // Gate enabling by both: animation time AND full page load
  let animationReady = false;
  let pageLoaded = false;
  const tryEnable = () => { if (animationReady && pageLoaded) enableContinue(); };

  // Give enough time for progress + status + prompt animations
  ...
  if (toodaBtn && toodaPanel) {
    ...
    const toggle = () => {
      ...
      if (open && citadelPanel) { // close Citadel if open
    ...
    // Populate team content in English
  ...
  // Citadel Botnet panel toggle and content
  if (citadelBtn && citadelPanel) {
    ...
    const toggleC = () => {
      ...
      if (openC && toodaPanel) { // close TOoDA if open
        toodaPanel.classList.remove('show');
        toodaPanel.setAttribute('aria-hidden', 'true');
...
In mid-April I launch an attack on Telegram: 2/4 DCs plus web.telegram.org go down, and in under an hour Downdetector received about ~10k reports.`;
  ...
  // Extend Citadel story with Telegram outage details and add proof overlay
  if (citadelPanel) {
